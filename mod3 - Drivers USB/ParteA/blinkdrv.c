 ...
 * Edited by Ramón Costales and Jose Ignacio Daguerre
			  ...
			  size_t len, loff_t *off)
{
	...
	unsigned char ledscolor[NR_BYTES_LEDSCOLOR];
	char kbuf[MAX_SIZE], *buff, *parseled;
	unsigned int color;
	...
	memset(ledscolor,0,NR_BYTES_LEDSCOLOR);

	/* Fill up the message accordingly */
	message[0]='\x05';
	message[1]=0x00;

	buff = (char*)vmalloc(MAX_SIZE);
	strcpy(buff, kbuf);
	/* Parse the color */	
	while((parseled = strsep(&buff, ",")) != NULL){
		if(strcmp(parseled, "\n") == 0){}
		else if(sscanf(parseled, "%d:%x", &lednum, &color) == 2){
			if(lednum < 0 || lednum > 7){
				printk(KERN_ALERT "Blinkdrv: the led number must be between 0 and 7\n");
				retval = -EINVAL;
				goto out_error;	
			}
			else if(color < 0x0 || color > 0xffffff){
				printk(KERN_ALERT "Blinkdrv: the hexadecimal number must be between 0x000000 and 0xffffff\n");
				retval = -EINVAL;
				goto out_error;	
			}
			else{
				/* We store the color indexed by the led number in an auxiliar array */
				ledscolor[(lednum*NR_COLORS)]=((color>>16) & 0xff);
				ledscolor[(lednum*NR_COLORS)+1]=((color>>8) & 0xff);
				ledscolor[(lednum*NR_COLORS)+2]=(color & 0xff);
			}
		}
		else{
			printk(KERN_ALERT "Blinkdrv: usage <lednumber>:<color in hex>, ...\n");
			retval = -EINVAL;
			goto out_error;	
		}
	}
	...
	for (i=0;i<NR_LEDS;i++){
		...
		message[3] = ledscolor[(i*NR_COLORS)];
		message[4] = ledscolor[(i*NR_COLORS)+1];
		message[5] = ledscolor[(i*NR_COLORS)+2];
